  ...
  return (
    <div className="space-y-6">
      <div className="card p-6">
        ...
        <p className="mt-4 text-sm text-pli-slate">Assessment questions in this domain: {itemCount}</p>
        <p className="mt-4 text-4xl font-semibold text-pli-teal">{score === null ? '—' : `${score.toFixed(1)}/10`}</p>
        {score !== null && <p className="mt-2 text-sm text-pli-slate">Current band: {scoreBandLabel(scoreBand(score))}</p>}
      </div>

      <div className="card p-6">
        <p className="text-xs uppercase tracking-[0.16em] text-pli-gold">Trend in this rule</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {interventions.map(intervention => (
          <div key={intervention.id} className="card p-6">
            ...
            <div className="mt-4 space-y-3 rounded-2xl bg-pli-bg p-4 text-sm text-pli-slate">
